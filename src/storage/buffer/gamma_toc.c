...
gamma_toc_invalid(gamma_toc *toc, Size nbytes)
{
	...
	for (i = 0; i < nentry; ++i)
	{
		if (pg_atomic_read_u32(&toc->toc_entry[i].refcount) > 0)
			continue;

		if (toc->toc_entry[i].flags & TOC_ENTRY_INVALID &&
			toc->toc_entry[i].nbytes > nbytes)
		{
			Assert(pg_atomic_read_u32(&toc->toc_entry[i].refcount) == 0);
			return i + 1;
		}
	}
...
gamma_toc_merge(gamma_toc *toc, Size nbytes)
{
	...
	for (i = nentry; i > 0; i--)
	{
		...
		if (pg_atomic_read_u32(&tail_entry->refcount) > 0)
			return false;
		...
		if (tail_entry->flags & TOC_ENTRY_INVALID)
		{
			...
			Assert(pg_atomic_read_u32(&tail_entry->refcount) == 0);
		...
		*/
		for (j = 0; j < i - 1; j++)
		{
			...
			if (pg_atomic_read_u32(&target_entry->refcount) > 0)
				continue;
...
gamma_toc_lru(gamma_toc *toc, double percent)
{
	...
	while (((double)count)/((double) nentry) < percent && ptr > 0)
	{
		...
		if (cur_entry->flags & TOC_ENTRY_INVALID)
		{
		...
		if (pg_atomic_read_u32(&cur_entry->refcount) > 0)
		{
			ptr = cur_entry->fifo_prev;
			count++;
			continue;
...
retry:
	...
	if (nentry != 0)
	{
		...
		Assert(result->flags & TOC_ENTRY_INVALID);
		result->flags = result->flags & (~TOC_ENTRY_INVALID);
...
gamma_toc_invalid_rel(gamma_toc *toc, Oid relid)
{
	...
	for (i = 0; i < nentry; ++i)
	{
		...
		retry = 0;
		while (pg_atomic_read_u32(&cur_entry->refcount) > 0)
		{
			if (retry++ == 10000)
			{
